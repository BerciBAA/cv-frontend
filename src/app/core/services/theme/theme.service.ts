...
function normalizeComponentTheme(t: Partial<ComponentThemeConfig> | ComponentThemeConfig): NormalizedComponentTheme {
  return {
    bg: t.bg ?? '',
    bgHover: t.bgHover ?? '',
    text: t.text ?? '',
...
export class ThemeService {
  private themeSignal = signal<NormalizedAppTheme>(normalizeTheme(STYLES.light))
  private mode = signal<'light' | 'dark'>('light')

  componentTheme<K extends keyof AppTheme>(key: K): Signal<NormalizedComponentTheme> {
  ...
  updateComponentTheme<K extends keyof AppTheme>(key: K, partial: Partial<AppTheme[K]>) {
    this.themeSignal.update(current => ({
      ...current,
